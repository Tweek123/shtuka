...
const Quote = ({className, mode = 'defaut', text}: {className?: string, mode?: 'defaut'|'black' ,text: string}) => {
    if(mode === 'black') {
        return <div className={`border-8 border-[#EB2F5B] ${className}`}>
            ...
            <Typography className="text-lg leading-[3] uppercase whitespace-pre-line md:text-3xl" variant="h1" fontFamily="ArialBlack" color="text-white">{text}</Typography>
    ...
    return <div className={`relative flex flex-col border-8 border-[#EB2F5B] p-4 ${className}`}>
        <Typography className="text-lg leading-[3] uppercase whitespace-pre-line md:text-3xl" variant="h1" fontFamily="ArialBlack" color="text-[#332f2f]">{text}</Typography>
